...
function Request(method, path, timeout, ensureHttps) {
  var usePrivateToken = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
...
exports.ListUserDetailViews = __webpack_require__(18).ListUserDetailViews;
...
var ApiClient = function () {
   ...
   */
  function ApiClient(databaseId, publicToken, privateToken, options) {
  ...
  _createClass(ApiClient, [{
    ...
    value: function send(request, callback) {
      ...
      var url = (this.useHttps ? 'https://' : 'http://') + (request.usePrivateToken ? this.privateBaseUri : this.baseUri) + signedUrl;
    ...
    key: '_signUrl',
    value: function _signUrl(req_part, usePrivateToken) {
      if (usePrivateToken) {
        ...
        var _url = '/' + this.databaseId + req_part;
        _url += (req_part.indexOf("?") == -1 ? "?" : "&") + "frontend_timestamp=" + parseInt(new Date().getTime() / 1000);

        var _shaObj = new jsSHA("SHA-1", "TEXT");
        _shaObj.setHMACKey(this.publicToken, "TEXT");
        _shaObj.update(_url);

        _url += "&frontend_sign=" + _shaObj.getHMAC("HEX");
        return _url;
      }
    }
  }]);
...
var ListUserDetailViews = function (_rqs$Request) {
  _inherits(ListUserDetailViews, _rqs$Request);
  ...
  function ListUserDetailViews(userId) {
    _classCallCheck(this, ListUserDetailViews);

    return _possibleConstructorReturn(this, (ListUserDetailViews.__proto__ || Object.getPrototypeOf(ListUserDetailViews)).call(this, 'GET', '/users/' + encodeURIComponent(userId) + '/detailviews/', 9000, false, true));
  }
  ...
  _createClass(ListUserDetailViews, [{
    key: 'bodyParameters',
    value: function bodyParameters() {
      var params = {};
